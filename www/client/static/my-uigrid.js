  ...
  module.service('uiGridExpandableService', ['gridUtil', '$compile', function (gridUtil, $compile) {
    var service = {
      ...
      toggleRowExpansion: function (grid, row) {
        ...
        if (row.isExpanded) {
          row.height = row.grid.options.rowHeight + angular.isFunction(grid.options.expandableRowHeight) ? grid.options.expandableRowHeight(row) : grid.options.expandableRowHeight;
        }
        else {
...
angular.module('ui.grid').run(['$templateCache', function($templateCache) {
  ...
  $templateCache.put('ui-grid/expandableRow',
    ...
    "     , height: row.height + 'px'}\"></div>"
  );


  $templateCache.put('ui-grid/expandableRowHeader',
    "<div class=\"ui-grid-row-header-cell ui-grid-expandable-buttons-cell\"><div class=\"ui-grid-cell-contents\"><i ng-class=\"{ '' : !row.isExpandable, 'ui-grid-icon-plus-squared' : !row.isExpanded, 'ui-grid-icon-minus-squared' : row.isExpanded }\" ng-click=\"grid.api.expandable.toggleRowExpansion(row.entity)\"></i></div></div>"
  );
  ...
  $templateCache.put('ui-grid/expandableScrollFiller',
    ...
    "              height: row.height + 2 + 'px', 'margin-left': grid.options.rowHeader.rowHeaderWidth + 'px' }\"><i class=\"ui-grid-icon-spin5 ui-grid-animate-spin\" ng-style=\"{ 'margin-top': ( row.height/2 - 5) + 'px',\n" +
